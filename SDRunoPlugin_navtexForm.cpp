...
void	SDRunoPlugin_navtexForm::Run() {
	show();
	nana::exec();
}

// Create the initial plugin form
void	SDRunoPlugin_navtexForm::Setup() {
	...
	hModule = GetModuleHandle(L"SDRunoPlugin_navtex");
	hdc = GetDC(NULL);
	...
	*(BITMAPINFO*)(settoverPixels + sizeof(BITMAPFILEHEADER)) = bmInfo_sett_over;
	...
	title_bar_label.caption("< bold size = 6 color = 0x000000 font = \"Verdana\">navtex decoder</>");
...
// TODO: Extra Form code goes here
	navtexAfcon. push_back ("afc off");
	navtexAfcon. push_back ("afc on");
...
void	SDRunoPlugin_navtexForm::navtex_showStrength	(float f) {
	navtexStrength. caption (std::to_string (f));
}

void	SDRunoPlugin_navtexForm::navtex_showCorrection	(float f) {
	navtexCorrection. caption (std::to_string (f));
}

void	SDRunoPlugin_navtexForm::navtex_showText	(const std::string &s) {
	navtexTextBox. caption (s);
}

void	SDRunoPlugin_navtexForm::set_navtexAfcon	(const std::string &s) {
	m_parent. set_navtexAfcon (s);
}

void	SDRunoPlugin_navtexForm::set_navtexReverse	(const std::string &s) {
	m_parent. set_navtexReverse (s);
}

void	SDRunoPlugin_navtexForm::set_navtexFecError	(const std::string &s) {
